# 붙여 넣는 프롬프트 — 학생 휴대폰까지 쓸 때 (live 모드)

## 먼저: 정말 이 문서가 맞습니까?

붙이는 방법이 두 가지입니다. 고르는 기준은 **학생 휴대폰이 필요한가** 하나뿐입니다.

| | 화면 하나에서만 | 학생 휴대폰까지 |
|---|---|---|
| 쓰는 문서 | **[integration-prompt-local.md](integration-prompt-local.md)** | 이 문서 |
| 부모 앱에 서버가 | 없어도 된다 (정적 사이트도 가능) | 있어야 한다 |
| 미리 등록할 것 | **없다** | 공유 비밀 + 주소 등록 (아래 0절) |
| 학생이 하는 일 | 화면을 본다 | 자기 기기로 들어와 자기 구슬을 본다 |
| 결과를 서버가 확인해 주나 | 아니오 | 예 (서명된 webhook) |

**사다리타기·모둠 발표자 뽑기를 대신하는 용도라면 왼쪽입니다.** 준비할 것이 없습니다.
이 문서는 학생 기기까지 끌어들일 때만 필요합니다.
